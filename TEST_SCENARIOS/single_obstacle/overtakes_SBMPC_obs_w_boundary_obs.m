scenario_name = 'overtakes_SBMPC_obs_w_boundary_obs';

%% Own ship
eta_initial = [-2500;0;0];      %[x, y, psi (rad)]
nu_initial  = [5;0;0];          %[u,v,r]
initial_state = [eta_initial;nu_initial];
waypoints_own_ship = [ -2500,  0;
                      10000,   0]; 
...
% SBMPC obs 1
initial_SBMPC_obstacle_ship_states(:,1) = [0;0;0*deg2rad;0.5;0;0]; %[x, y, psi, u, v, r]
waypoints_SBMPC_obstacle_ships(:,:,1) = [ -0, -0;   %obstacle ship 1 waypoints [ 3000, 3000;   %obstacle ship 1 waypoints
                                          3000,0];
SBMPC_obs_speed_between_waypoints(1,:) =  [0.5];
...
boundary_obstacles_params = [-3700, 400, 3000, 400]; %[start_x, start_y, end_x, end_y]. One row for each obstacle!!

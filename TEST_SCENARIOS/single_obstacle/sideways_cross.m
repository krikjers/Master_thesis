scenario_name = 'sideways_cross';

%% Own ship
eta_initial = [-2475;-2475;45*deg2rad];      %[x, y, psi (rad)]
nu_initial  = [5;0;0];          %[u,v,r]
initial_state = [eta_initial;nu_initial];
waypoints_own_ship = [ -2475,     -2475;
                      3000,   3000];
...
% 
% % obs 2
% initial_obstacle_ship_states(:,2) = [2000;-200;(-180)*deg2rad;5;0;0];
% waypoints_obstacle_ships(:,:,2)   = [2000, -200;    %obstacle ship 2 waypoints 
%                                     -2000, -200];    
...
boundary_obstacles_params = [-2880, -1800, -700, 200]; %[start_x, start_y, end_x, end_y]. One row for each obstacle!!
circle_obstacle_params    = [80, -1560, -460;
                             100, 600,800];          %[radius, center_x, center_y].     One row for each obstacle!!

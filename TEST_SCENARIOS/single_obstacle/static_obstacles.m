scenario_name = 'static_obstacles';
...
HEAD_ON_os_relative_to_SBMPC_obs(1,:) = [1]; 
...
% SBMPC obs 1
initial_SBMPC_obstacle_ship_states(:,1) = [6000;-6000;-180*deg2rad;5;0;0]; %[x, y, psi, u, v, r]
waypoints_SBMPC_obstacle_ships(:,:,1) = [ 6000, -6000;   %obstacle ship 1 waypoints [ 3000, 3000;   %obstacle ship 1 waypoints
                                         -8000, -6000];
...
boundary_obstacles_params = [1000, -350, 1000, 9000]; %[start_x, start_y, end_x, end_y]. One row for each obstacle!!
circle_obstacle_params    = [250, -1000, 0];          %[radius, center_x, center_y].     One row for each obstacle!!
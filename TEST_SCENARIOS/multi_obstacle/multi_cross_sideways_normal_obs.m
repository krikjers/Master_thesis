scenario_name = 'multi_cross_sideways_normal_obs';

%% Own ship
eta_initial = [-1414;1414;-45*deg2rad];      %[x, y, psi (rad)]
nu_initial  = [5;0;0];          %[u,v,r]
initial_state = [eta_initial;nu_initial];
waypoints_own_ship = [ -1414, -1414;
                       4000,   -4000]; 
...
% obs 1
initial_obstacle_ship_states(:,1) = [-1414;-1414;45*deg2rad;5;0;0]; %[x, y, psi, u, v, r]
waypoints_obstacle_ships(:,:,1)   = [ -1414, -1414;    %obstacle ship 1 waypoints
                                    5000,5000];
...
% obs 2
initial_obstacle_ship_states(:,2) = [1500;0;-135*deg2rad;5;0;0];
waypoints_obstacle_ships(:,:,2)   = [1500, 0;    %obstacle ship 2 waypoints 
                                    -1500, -3000];    
...
% SBMPC obs 1
initial_SBMPC_obstacle_ship_states(:,1) = [3500;0;-180*deg2rad;5;0;0]; %[x, y, psi, u, v, r]
waypoints_SBMPC_obstacle_ships(:,:,1) = [ 3500, 0;   %obstacle ship 1 waypoints [ 3000, 3000;   %obstacle ship 1 waypoints
                                         -8000, -0];
SBMPC_obs_speed_between_waypoints(1,:) =  [5];
ROLE_SBMPC_relative_to_own_ship(1) = 1;     % 1 = give way
...
boundary_obstacles_params = [-100, -3000, 1200, -1000]; %[start_x, start_y, end_x, end_y]. One row for each obstacle!!
circle_obstacle_params    = [350, 0, 0];          %[radius, center_x, center_y].     One row for each obstacle!!

scenario_name = 'ho_overtakes_sbmpc_crossing_3_mixed_obs';

%% Own ship
eta_initial = [-4200;0;0];      %[x, y, psi (rad)]
nu_initial  = [5;0;0];          %[u,v,r]
initial_state = [eta_initial;nu_initial];
waypoints_own_ship = [ -4200,  0;
                      10000,   0]; 
                  


ROLE_os_relative_to_SBMPC_obs(1,:) = [1];  %role value of 1 means that  os is give way vessel relative to sbmpc obs i and therefroe sbmpc obs i is stand on.
...
% obs 1
initial_obstacle_ship_states(:,1) = [5000;0;-180*deg2rad;5;0;0]; %[x, y, psi, u, v, r]
waypoints_obstacle_ships(:,:,1)   = [ 5000, 0;    %obstacle ship 1 waypoints
                                      2500, 0;
                                      -5000, 5000];
...
HEAD_ON_relative_to_SBMPC_obs(1,:) = [1]; 



% obs 2
initial_obstacle_ship_states(:,2) = [1500,1500,-90*deg2rad,2,0,0];
waypoints_obstacle_ships(:,:,2)   = [1500, 1500;    %obstacle ship 2 waypoints 
                                     1500, -4000;
                                    1500, -4000];    
                               %WHEN DONE TESTING, FIRST WAYPOINT CAN BE SUBSTITUTED BY INITIAL OBS POSITION
obs_speed_between_waypoints(2,:) =  [2,2];
changing_obs(2) = 0;
...
% SBMPC obs 1
initial_SBMPC_obstacle_ship_states(:,1) = [0;0;0*deg2rad;2;0;0]; %[x, y, psi, u, v, r]
waypoints_SBMPC_obstacle_ships(:,:,1) = [ -0, -0;   %obstacle ship 1 waypoints [ 3000, 3000;   %obstacle ship 1 waypoints
                                          4000,0;
                                          4000,0];
SBMPC_obs_speed_between_waypoints(1,:) =  [2,2];

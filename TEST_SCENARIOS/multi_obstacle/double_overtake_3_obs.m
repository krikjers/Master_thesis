scenario_name = 'double_overtake_3_obs';

%% Own ship
eta_initial = [-3500;50;0];      %[x, y, psi (rad)]
nu_initial  = [2.5;0;0];          %[u,v,r]
initial_state = [eta_initial;nu_initial];
waypoints_own_ship = [ -3500,     50;
                      10000,   50]; 
                  

ROLE_os_relative_to_SBMPC_obs(1,:)    = [1,0,1];  %role value of 1 means that  os is give way vessel relative to sbmpc obs i
HEAD_ON_os_relative_to_SBMPC_obs(1,:) = [0,0,1]; 
...
% obs 1
initial_obstacle_ship_states(:,1) = [-9000;-9000;0*deg2rad;1;0;0]; %[x, y, psi, u, v, r]
waypoints_obstacle_ships(:,:,1)   = [ -9000, -9000;    %obstacle ship 1 waypoints
                                     1000,-9000];
...
% SBMPC obs 1
initial_SBMPC_obstacle_ship_states(:,1) = [-2300;0;0*deg2rad;1;0;0]; %[x, y, psi, u, v, r]
waypoints_SBMPC_obstacle_ships(:,:,1) = [ -2300, 0;   
                                          5000, 0];
SBMPC_obs_speed_between_waypoints(1,:) =  [1];
ROLE_SBMPC_relative_to_own_ship(1) = 0;
ROLE_SBMPC_relative_to_SBMPC_obs(1,:) = [-1,0,1]; 
HEAD_ON_SBMPC_relative_to_own_ship(1) = 0; 
HEAD_ON_SBMPC_relative_to_SBMPC_obs(1,:) = [-1,0,1]; 


%SBMPC obs 2
initial_SBMPC_obstacle_ship_states(:,2) = [-6000;100;0*deg2rad;5;0;0];
waypoints_SBMPC_obstacle_ships(:,:,2)   = [-6000, 100;    %obstacle ship 2 waypoints 
                                           5000, 100];    
...
ROLE_SBMPC_relative_to_SBMPC_obs(2,:) = [1,-1,1];
HEAD_ON_SBMPC_relative_to_own_ship(2) = 0; 
HEAD_ON_SBMPC_relative_to_SBMPC_obs(2,:) = [0,-1,1];



%SBMPC obs 3
initial_SBMPC_obstacle_ship_states(:,3) = [3500;0;-180*deg2rad;5;0;0];
waypoints_SBMPC_obstacle_ships(:,:,3)   = [3500, 0;    %obstacle ship 2 waypoints 
                                           -5000, 0];    
                               %WHEN DONE TESTING, FIRST WAYPOINT CAN BE SUBSTITUTED BY INITIAL OBS POSITION
SBMPC_obs_speed_between_waypoints(3,:) =  [5];
ROLE_SBMPC_relative_to_own_ship(3) = 1;
ROLE_SBMPC_relative_to_SBMPC_obs(3,:) = [1,1,-1];
HEAD_ON_SBMPC_relative_to_own_ship(3) = 0; 
HEAD_ON_SBMPC_relative_to_SBMPC_obs(3,:) = [1,1,-1];
